...
import 'nonprePriorityScheduler.dart';
class nonPrePriority extends StatefulWidget {
  const nonPrePriority({super.key});

  @override
  State<nonPrePriority> createState() => _nonPrePriorityState();
}

class _nonPrePriorityState extends State<nonPrePriority> {
  ...
  void startsimulation() {
    ...
    List<Map<String, dynamic>> processList = processes.map((process) {
      return {
        ...
        "Priority": int.tryParse(process["Priority"]!.text) ?? 1000,
      };
    }).toList();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Non Premptive Priority Scheduling"),
